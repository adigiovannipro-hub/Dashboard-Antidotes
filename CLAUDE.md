...
| Production (cartes client) | cartes de `/`, `src/lib/production/`, `src/lib/prompts/*.md`, `/api/generate/[phase]`, `/api/jobs/[id]` | 3 tables (`client_phases`, `generation_jobs`, `wording_history` — le « client » du cahier des charges est un espace : `workspace_id` + `org_id`, RLS owner-only). Cycle décalé à 4 phases : pendant le mois M, intentions/wording/programmation visent M+1 et reporting M−1 — le cahier des charges faisait viser « le mois courant » à wording/programmation, incompatible avec ses propres fenêtres ; bascule reporting → intentions à la génération ou le 15. Carte cockpit : 4 segments (vert = fait ou en cours, ambre = fenêtre ouverte ou dépassée, gris = pas encore), badge du retard le plus ancien, mesures par phase, avancement chiffré, encart d'alerte, bouton à compteur, menu `…` (4 actions + liens ; « Ouvrir le contexte » attend le prompt 2). Jobs en `claude-sonnet-4-6` (demandé explicitement — les autres modules restent en opus) depuis des **prompts markdown éditables sans code**, tracés pour Vercel dans `next.config.ts` ; wording par lots de 4, reprise des seuls échecs, anti-répétition via `wording_history` ; intentions insérées dans le planning (mois, réseaux, colonne « Intention » — jamais dans `wording`, qui reste la caption et le compteur) ; reporting markdown sur les volumes du planning (pas de chiffres de régies : il le dit) ; **programmation en stub** qui journalise sans rien clôturer. Traitement post-réponse par `after()`, poll 3 s, jobs muets > 10 min soldés en erreur. `getClientContext` renvoie vide jusqu'au prompt 2. Sans les tables, la carte dit « Cycle indisponible » et retire son bouton plutôt que d'affirmer un retard qu'elle ne sait pas — l'erreur Supabase (`42P01`/`PGRST205`) est testée, contrairement à la règle habituelle des `queries.ts`. Migrations 0032-0033 **pas encore appliquées à la vraie base** : `supabase/seeds/rattrapage-production.sql` les pose d'un seul collage, convergent (il répare un passage interrompu, refuse de toucher une table peuplée d'une autre forme) |
