...
Les six modules ont leur suite : `tests/isolation.test.ts` (socle), `planning-isolation`, `receipts-isolation`, `finance-isolation`, `mon-travail-isolation`, `contexte-isolation` (owner-only : le client ne lit **rien**, pas même son propre espace). Les sept ont **toutes tourné vertes contre le vrai projet Supabase** le 12/08/2026 (run db-admin 31556558795, 91 tests) — elles sautent toujours en local tant que `.env.local` n'est pas renseigné, et la CI de push ne les exécute jamais : le vert d'un push ne prouve rien, celui de db-admin si.
