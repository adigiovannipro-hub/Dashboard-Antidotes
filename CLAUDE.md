...
| Finance (phase 2.1) | `/entreprise/finance`, `src/lib/finance/` | 8 tables, bande de mesures (disponible masquable à l'œil, à facturer, en retard, dépensé), facturation à venir (lue du module Échéances), courbe solde vert + dépenses rouges 7/30/90 j, dépenses carte avec pastilles d'initiales et filtres de date JJ/MM/AAAA. **Synchronisation réelle en route** : `airwallex-sync.yml` horaire depuis GitHub (Airwallex bloque les IP Vercel), premier passage réussi le 7 août — 46 soldes, 100 dépenses, 30 factures. Les factures Airwallex restent en base mais ne pilotent plus l'écran (noms de clients faux). Purge de la démo : `pnpm purge:finance-demo` ou Actions → Base de données |
| Échéances (nouveau) | `/entreprise/echeances`, `src/lib/billing/` | 2 tables (`billing_engagements`, `billing_installments`), remplaçant du board Monday : un devis signé engendre ses échéances mensuelles, facturées le lendemain de la fin du mois de prestation. Écran complet (à émettre, mois par mois, engagements, historique), Server Actions, RLS + `tests/billing-isolation.test.ts`. Migrations 0016-0017 **pas encore appliquées à la vraie base** |
