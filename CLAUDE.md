...
| Échéances | `/entreprise/echeances`, `src/lib/billing/` | 2 tables (`billing_engagements`, `billing_installments`), remplaçant du board Monday reconstruit dans ses groupes de statut : À facturer / Facturée / Payée (repliée) / Devis confirmé, sommes HT-TTC par groupe, archivé replié en bas façon « Mon travail ». Courbe prévisionnelle 3/6/12 mois en tête (mensualités des devis seulement, même UI que la courbe Finance). Un devis se saisit une fois (période début → fin, total **ou** mensuel HT, TVA en liste — 0 % par défaut, aperçu de la division) et ses mensualités s'ajustent ligne à ligne. **Les factures Airwallex sans devis s'affichent aussi** (lignes « Facture hors devis », montant unique en HT et TTC, archivage dérivé à 60 j, zéro action — leur statut est celui d'Airwallex). Les statuts avancent seuls : bascule « à facturer » dérivée de la date (zéro cron), puis rapprochement avec les factures Airwallex du module Finance — étape `billing` de `runFinanceSync`, donc du sync GitHub horaire : client normalisé + TTC au centime + fenêtre de dates, statut qui n'avance jamais en arrière, archivage auto à 60 j (`src/lib/billing/reconcile.ts`, pur et testé). Boutons de ligne = filet manuel. Migrations appliquées à la vraie base jusqu'à **0025** (workflow db-admin joué le 9 août depuis la branche du chantier) ; le rapprochement horaire ne tournera en production qu'au merge — le sync s'exécute depuis `main` |
