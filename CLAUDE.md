...
| Planning Éditorial | `/espace/[workspace]/planning`, `src/lib/planning/` | 8 tables — le board Monday **natif et éditable**, plus un miroir : mois → réseaux → publications, cellules éditables, colonnes personnalisables et redimensionnables (0009), étiquettes libres « + Nouvelle étiquette » (0029), journal d'activité, recherche ⌘F (sujets + wordings), panneau latéral animé avec carrousel sur fond flouté, visionneuse plein écran, retours avec adresses e-mail taguées envoyés via la boîte Gmail des Reçus (0030), sélection multiple **persistante** avec duplication. Import Monday (`pnpm import:monday`) et analyses strategy / cadence / health toujours là. **0009, 0029 et 0030 manquent encore sur la vraie base** — c'est la cause des erreurs `planning_columns` et de l'onglet Activités muet ; un fichier de rattrapage idempotent (transmis dans la conversation) les applique d'un seul collage et prévient PostgREST (`notify pgrst`) |
