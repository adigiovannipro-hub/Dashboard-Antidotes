...
| Échéances | `/entreprise/echeances`, `src/lib/billing/` | 2 tables (`billing_engagements`, `billing_installments`), remplaçant du board Monday reconstruit dans ses groupes de statut : À facturer / Facturée / Payée (repliée) / Devis confirmé, sommes HT-TTC par groupe, archivé replié en bas façon « Mon travail ». Un devis se saisit une fois (période début → fin, total **ou** mensuel HT, TVA, aperçu de la division) et ses mensualités s'ajustent ligne à ligne. Les statuts avancent seuls : bascule « à facturer » dérivée de la date (zéro cron), puis rapprochement avec les factures Airwallex du module Finance — étape `billing` de `runFinanceSync`, donc du sync GitHub horaire : client normalisé + TTC au centime + fenêtre de dates, statut qui n'avance jamais en arrière, archivage auto à 60 j (`src/lib/billing/reconcile.ts`, pur et testé). Boutons de ligne = filet manuel. Migrations 0016-0017 et **0025 pas encore appliquées à la vraie base** ; l'écran, les actions et le rapprochement ont été vérifiés de bout en bout sur banc local (Postgres 16 + PostgREST) |
