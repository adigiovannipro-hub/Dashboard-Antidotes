 ...
 * pilier est une sous-carte (nom, description, objectif business, appels à
 * l'action autorisés, formats, angles, fréquence), sauvegardée au blur comme
 * le reste du brief.
 */
...
const toDraft = (pillar: ContextPillar): PillarDraft => ({
  ...
  objectifBusiness: pillar.objectif_business ?? "",
  ctaText: (pillar.cta_autorises ?? []).join(", "),
...
const fromDraft = (draft: PillarDraft): ContextPillar => ({
  ...
  objectif_business: draft.objectifBusiness.trim(),
  cta_autorises: splitList(draft.ctaText),
...
export function PillarEditor({
  ...
  function persist(next: PillarDraft[]) {
    ...
    const cleaned = next
      .map(fromDraft)
      .filter(
        (pillar) => pillar.nom.length > 0 || pillar.description.length > 0,
      );
            ...
            >
              <div className="flex items-center justify-between gap-2">
                <input
                  ...
                  onChange={(event) =>
                    update(index, { nom: event.target.value })
                  }
              ...
              <label className="flex flex-col gap-1">
                ...
                <input
                  value={draft.objectifBusiness}
                  disabled={readOnly}
                  placeholder="prises de rendez-vous"
                  aria-label={`Objectif business du pilier ${index + 1}`}
                  onChange={(event) =>
                    update(index, { objectifBusiness: event.target.value })
                  }
              ...
              <label className="flex flex-col gap-1">
                {/* Liste fermée : la génération n'a le droit d'écrire que
                      ces appels à l'action, jamais d'en inventer un de plus. */}
                <span className="type-overline text-text-secondary">
                  CTA autorisés
                </span>
                <input
                  value={draft.ctaText}
                  disabled={readOnly}
                  placeholder="Prendre rendez-vous, Découvrir la collection"
                  aria-label={`CTA autorisés du pilier ${index + 1}`}
                  onChange={(event) =>
                    update(index, { ctaText: event.target.value })
                  }
                  className={FIELD_CLASS}
                />
              </label>
              ...
              <label className="flex flex-col gap-1">
                ...
                <input
                  ...
                  onChange={(event) =>
                    update(index, { formatsText: event.target.value })
                  }
                  className={FIELD_CLASS}
                />
              </label>
              ...
              <label className="flex flex-col gap-1">
                ...
                <input
                  ...
                  onChange={(event) =>
                    update(index, { anglesText: event.target.value })
                  }
                  className={FIELD_CLASS}
                />
              </label>
              ...
              <label className="flex flex-col gap-1">
                ...
                <input
                  ...
                  onChange={(event) =>
                    update(index, { frequence: event.target.value })
                  }
                  className={FIELD_CLASS}
                />
              </label>
      ...
      {!readOnly ? (
        <div className="mt-5">
          <Button
            ...
            onClick={() =>
              setDrafts((current) => [...current, toDraft(EMPTY_PILLAR)])
            }
          >
