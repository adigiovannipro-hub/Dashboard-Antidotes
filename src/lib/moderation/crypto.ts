...
function readKey(): Buffer {
  ...
  if (!raw) {
    throw new Error(
      "CREDENTIALS_ENCRYPTION_KEY absent. Générer avec : openssl rand -base64 32",
    );
  }
  const key = Buffer.from(raw, "base64");
  if (key.length !== KEY_LENGTH) {
    throw new Error(
      `CREDENTIALS_ENCRYPTION_KEY doit faire 32 octets une fois décodée, ${key.length} trouvés.`,
    );
  }
  return key;
}
